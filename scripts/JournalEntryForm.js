import { getMoods, saveEntry, useMoods, getLastEntryId } from "./JournalDataProvider.js"
import { saveTagEntry } from './tags/TagFilter.js'
...
const entryCheck = (newEntry, tagsArray) => {
    ...
    if (newEntry.date !== "" && newEntry.concept.length > 0 && newEntry.concept.length < 25 && newEntry.entry.length > 0 && newEntry.entry.length < 200 && newEntry.moodId !== 0) {
        saveEntry(newEntry)
        .then(
            saveTagEntry(tagsArray, getLastEntryId())
        )
    }
}